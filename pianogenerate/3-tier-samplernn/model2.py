    ...
    with tf.name_scope(name):
      ...
      with tf.name_scope('loss'):
        ...
        if l2_regularization_strength is None or l2_regularization_strength == 0:
          ...
          return total_loss, accuracy , final_big_frame_state, final_frame_state
    ...
    with tf.name_scope('infe_para'):
      ...
      infe_para['infe_big_frame_inp'] = \
        tf.get_variable("infe_big_frame_inp",
                        [self.batch_size, self.big_frame_size, 1], dtype=tf.float32)
      infe_para['infe_big_frame_outp'] = \
        tf.get_variable("infe_big_frame_outp",
                        [self.batch_size, self.big_frame_size / self.frame_size, self.dim], dtype=tf.float32)
      ...
      infe_para['infe_big_frame_state'] = self.big_cell.zero_state(self.batch_size, tf.float32)
      infe_para['infe_frame_state'] = self.cell.zero_state(self.batch_size, tf.float32)

      infe_para['infe_big_frame_outp'], \
      infe_para['infe_final_big_frame_state'] = \
        self._create_network_BigFrame(num_steps=1,
